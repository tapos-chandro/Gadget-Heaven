...
const ProductDetails = () => {
  ...
  const { addCart, setAddCart , wishList, setWishList} = useContext(ContextProvider);
  ...
  const handleWishList = (pdId) => {
    const findPd = wishList.find(pd =>pd.id === pdId)
  ...
  console.log(addCart, 'added to cart')
          ...
          <div className=" grid grid-cols-3 gap-8">
            ...
            <div className="col-span-3 md:col-end-2 lg:col-span-2 gap-5">
              <div className="flex flex-col gap-4">
                <h1 className="text-3xl font-semibold text-black">{findProduct?.title}</h1>
                <p className="text-xl text-gray-600">Price: {findProduct?.price}tk</p>
                <div>
                  <button className=" ring-2 rounded-full px-2 text-base text-green-400 ring-green-400">
                    In Stock
                  </button>
                </div>
                <p className="text-xl text-gray-500">{findProduct?.description}</p>
                <div>
                ...
                <div className="my-3 flex gap-3 ">
                  <button onClick={handleAddToCart} className="flex items-center gap-2 text-xl text-white px-5 bg-[#9538E2] p-2 rounded-full">
                    Add To Cart <IoCart />
                  </button>
                  <button
                    className="rounded-full ring-1 text-xl ring-gray-300 px-2.5"
